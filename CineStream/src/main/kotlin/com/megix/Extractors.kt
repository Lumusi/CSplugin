...
open class Driveleech : ExtractorApi() {
    ...
    // Sep-2026: driveseed file pages mint the real link via generate(), which needs
    ...
    private suspend fun webViewGenerate(fileUrl: String, action: String): String? {
        ...
        return try {
            withContext(Dispatchers.IO) {
                            ...
                            override fun onPageFinished(view: WebView?, url: String?) {
                                view?.postDelayed({
                                    view.evaluateJavascript(
                                        "(function(){try{if(typeof generate!=='function')return Android.onResult('NOAPI');generate('$action').then(r=>Android.onResult(JSON.stringify(r))).catch(e=>Android.onResult('ERR:'+e));}catch(e){Android.onResult('THROW:'+e)}})()",
                                        null
                                    )
                                }, 9000)
                            }
                ...
                raw?.let { parseGenerateUrl(it) }
            }
        } catch (e: Exception) {
            Log.d("Driveseed", "webViewGenerate: $e")
            null
        }
    }

    private fun parseGenerateUrl(json: String): String? {
        return try {
            val u = JSONObject(json).optString("url")
            if (u.isBlank()) {
                Log.d("Driveseed", "generate: ${json.take(160)}")
                null
            } else if (u.contains("drive.google.com/file/d/")) {
                val id = Regex("""drive\.google\.com/file/d/([^/]+)""").find(u)?.groupValues?.get(1)
                if (id != null) "https://drive.google.com/uc?export=download&id=$id" else u
            } else u
        } catch (e: Exception) {
            null
        }
    ...
    override suspend fun getUrl(
        ...
        // Sep-2026 driveseed generate() API: Turnstile-gated, only a real page
        // session can mint it. The static hrefs below are rotating decoys
        // (HTTP 500 once stale), so try the in-page API first via WebView.
        if (document.html().contains("action_token")) {
            try {
                webViewGenerate(url, "original")?.let { finalUrl ->
                    myCallback(finalUrl, "[Generate]")
                    return
                }
            } catch (e: Exception) {
                Log.d("Driveseed", "generate(original): $e")
            }
        }
